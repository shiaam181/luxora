    ...
    <div className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-xl transition-all duration-300">
      <div className="relative cursor-pointer" onClick={() => onViewDetails && onViewDetails(product)}>
        ...
        <button
          ...
          className={`absolute top-2 right-2 p-2 rounded-full ${inWishlist ? 'bg-red-500 text-white' : 'bg-white text-gray-700'} hover:scale-110 transition shadow-lg`}
        >
          {isTogglingWishlist ? (
        ...
        {/* Bottom Right - Best Seller Badge */}
        {product.tags?.includes('bestseller') && (
          <span className="absolute bottom-2 right-2 bg-yellow-500 text-white px-2 py-1 rounded text-xs font-semibold flex items-center">
      ...
      <div className="p-2 md:p-4">
        {/* Category Badge - Mobile Style */}
        <span className="inline-block bg-gray-100 text-gray-600 text-xs px-2 py-0.5 rounded mb-1">
          {product.category}
        </span>
        
        {/* Product Name - Compact */}
        <h3 className="text-sm md:text-lg font-semibold text-gray-800 mb-1 line-clamp-2 cursor-pointer hover:text-purple-600" 
            onClick={() => onViewDetails && onViewDetails(product)}>
          {product.name}
        </h3>
        
        {/* Price Section - Skechers Style */}
        <div className="mb-2">
          <div className="flex items-baseline gap-1 flex-wrap">
            ...
            {product.originalPrice > product.price && (
              <>
                ...
                <span className="text-xs md:text-sm text-red-600 font-semibold">{discount}% OFF</span>
              </>
            )}
          </div>
        </div>
        
        {/* Buttons - Compact Mobile Style */}
        <div className="flex gap-1.5">
          {showMoveToCart ? (
            <button
              onClick={() => moveWishlistToCart(product)}
              disabled={product.stock === 0}
              className={`flex-1 ${product.stock === 0 ? 'bg-gray-400' : 'bg-black hover:bg-gray-800'} text-white px-2 py-2 rounded text-xs md:text-sm font-semibold transition flex items-center justify-center`}
            >
            ...
            <>
              <button
                ...
                className={`flex-1 ${product.stock === 0 || isAddingToCart ? 'bg-gray-400' : 'bg-black hover:bg-gray-800'} text-white px-2 py-2 rounded text-xs md:text-sm font-semibold transition flex items-center justify-center`}
              >
                {isAddingToCart ? (
                  ...
                  <>
                    <ShoppingCart className="w-3 h-3 md:w-4 md:h-4 md:mr-1" />
                    <span className="hidden md:inline">{product.stock === 0 ? 'Out' : 'Add'}</span>
                  </>
                )}
              </button>
             </>
...
const ProductDetailsModal = ({ product: initialProduct, onClose }) => {
  ...
  // Get product images (custom or fallback)
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-0 md:p-4 overflow-y-auto">
        <div className="bg-white w-full h-full md:rounded-2xl md:max-w-6xl md:w-full md:max-h-[95vh] overflow-y-auto">
          
          {/* ✅ STICKY HEADER */}
          <div className="sticky top-0 bg-white border-b px-4 py-3 flex justify-between items-center z-20 shadow-sm">
            <h2 className="text-lg md:text-2xl font-bold text-gray-800 truncate flex-1">Product Details</h2>
            <button onClick={onClose} className="text-gray-600 hover:text-gray-800 p-2">
              <X className="w-6 h-6" />
            </button>
          </div>

          <div className="md:grid md:grid-cols-2 md:gap-8 p-4 md:p-8">
            
            {/* ✅ LEFT: CLEAN IMAGE GALLERY */}
            <div className="mb-6 md:mb-0">
              {/* Main Image Display */}
              <div className="relative mb-4 bg-gray-50 rounded-xl overflow-hidden border-2 border-gray-200">
...
/>
                    ...
                    <div className="w-full h-full bg-gradient-to-br from-purple-200 to-pink-200 flex items-center justify-center text-8xl">
                      ...
                      {product.image === 'kurti' && '👗'}
                      {product.image === 'earrings' && '💂'}
                      {product.image === 'handbag' && '👜'}
                      {product.image === 'saree' && '🥻'}
                      {product.image === 'sunglasses' && '🕶️'}
                      {product.image === 'ring' && '💎'}
                      {product.image === 'palazzo' && '👘'}
                      {!product.image && <Package className="w-32 h-32 text-purple-400" />}
                    </div>
                  )}
                </div>
                ...
                {product.tags?.includes('bestseller') && (
                  <span className="absolute top-4 right-4 bg-yellow-500 text-white px-3 py-1 rounded-full text-sm font-semibold flex items-center shadow-lg">
          ...
          <div className="w-full h-full bg-gradient-to-br from-purple-200 to-pink-200 flex items-center justify-center text-2xl">
            ...
            <div className="space-y-6">
              ...
              <div className="border-y py-4">
                ...
                <p className="text-green-600 font-semibold text-sm">(incl. of all taxes)</p>
              </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-4">
                  ...
                  <button
                    ...
                    className={`${inWishlist ? 'bg-red-500 text-white' : 'bg-gray-200 text-gray-700'} p-3 rounded-lg hover:scale-110 transition shadow-md`}
                  >
                ...
                <button
                  onClick={handleAddToCart}
                  disabled={product.stock === 0}
                  className={`w-full ${product.stock === 0 ? 'bg-gray-400 cursor-not-allowed' : 'bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700'} text-white px-6 py-4 rounded-lg font-bold text-lg transition flex items-center justify-center shadow-lg`}
                >
              ...
              <div className="border-t pt-6 space-y-3">
                ...
                <div className="flex items-center text-gray-700">
                  ...
                  <span className="text-sm">100% Safe & Secure Products</span>
                </div>
              </div>
            </div>
          </div>
                      ...
                      <div className="flex gap-2">
                        <button 
                          ...
                          onClick={() => {
                            ...
                            toast.success(editingReviewId ? 'Review updated!' : 'Thank you for your review!');
                            setReviewRating(0);
                            setReviewText('');
...
const ProductsPage = ({ setSelectedProduct, setShowCart }) => {
  const { products, categories: rawCategories } = useStore();
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-4 md:py-8 pb-20 md:pb-8">
      <div className="container mx-auto px-3 md:px-4">
        <h1 className="text-3xl md:text-4xl font-bold text-center mb-4 md:mb-8">Our Products</h1>
        ...
        <div className="max-w-2xl mx-auto mb-4 md:mb-8">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 md:py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-600"
            />
          </div>
        </div>
        ...
        {/* 📱 MOBILE STICKY BOTTOM BAR (Skechers Style) */}
        <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t-2 border-gray-200 shadow-lg z-30">
          <div className="flex items-center justify-around py-3 px-4">
            ...
            {/* Sort Button */}
            <button
              onClick={() => document.getElementById('sort-select-mobile')?.focus()}
              className="flex items-center gap-2 px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {showFilters && (
          <>
            ...
            <div className="fixed bottom-0 left-0 right-0 bg-white rounded-t-2xl z-50 max-h-[80vh] overflow-y-auto p-6 animate-slide-up">
              <div className="flex justify-between items-center mb-6">
